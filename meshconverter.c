...
int main(int argc, char** argv) {
    ...
    fread(&hxMeshData, 1, fsize(hxMeshFile), hxMeshFile);
    ...
    fread(&hxMeshData.location.localTransMtx, sizeof(float), 24, hxMeshFile);
    for (int i = 0; i < 12; i++) {
        if (i % 4 == 0) printf("\n");
        printf("%f ", hxMeshData.location.localTransMtx[i]);
    }
    for (int i = 0; i < 12; i++) {
        if (i % 4 == 0) printf("\n");
