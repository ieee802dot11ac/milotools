...
int main(int argc, char** argv) {
	...
	printf("\n");
	fread(&hxMeshData.transform.trans_count, 4, 1, hxMeshFile);
	printf("%i\n", hxMeshData.transform.trans_count);
	if (hxMeshData.transform.trans_count) { // i *do not* want to have to deal with these right now
		printf("fak\n"); 
		return -1;
	}
	...
	fread(&hxMeshData.transform.preserve_scale, 1, 1, hxMeshFile); 
	printf("%i\n", hxMeshData.transform.preserve_scale); // irrelevant for single files
